...
def main():
    ...
    PROMPT = st.sidebar.text_input("提示词", value="")
    ...
    if 'all_answers' not in session_state:
        ...
        你现在是我的语言模型数据集标注助手，请给以下内容进行数据集标注，你的目标是完成result的内容，示例：
        {"questionAnswers":"小麦：你有没有感到情绪沮丧或郁闷？被试者：没有。我最近感觉很好，感觉每天都有幸运的事发生在我身上。小麦：你会不会发现早晨是你最开心的时候？被试者：是的，我早上很开心。感觉整个人充满了活力。小麦：最近你有没有想要流泪的时候？被试者：没有，我很久没哭过了。",
        "result": "你的生活态度积极，你相信生活中还有很多机遇和挑战等着你，请继续保持下去。抑郁症似乎和你并没有关系"}
        """
    ...
    if selected_question:
        ...
        with col2:
            if st.button("科大讯飞星火生成回答"):
                session_state.generated_answer = ''
                try:

                    event_count = 0

                    session_state.generated_answer = test.XF_answers(prompt)
                    if event_count > 10:
                        event_count = 0
                        reset_text_area()
